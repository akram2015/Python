...
query = list(set(input("query: ").strip("").split())) # removes repeated items from the list and converting back to list from set

if(len(query)==1):                                    # searching for one item condition
    for i,quote in enumerate(data_list):
        found_at = quote.find(query[0])
        if( found_at > 0):
            print("Found at ", i, "..."+quote[found_at:found_at+50], "...")
   
elif ("or" in query) and ("and" not in query):        # searching for two items by applying OR condition
    query.remove("or")                                # Remove "or" from the list
    print("Performing OR search for: ", query)
    for i, quote in enumerate(data_list):
            if (query[0] in quote) or (query[1] in quote):
                print("Found at", i, quote[0:])
   
elif("and" in query) and ("or" not in query):         # searching for two items by applying AND condition
    query.remove("and")                          
...
elif("and" in query) and ("or" in query):             # when the user enter both operations, searching for two items will apply AND condition
    query.remove("or")
    print("Performing AND search for: ", query)
    for i,quote in enumerate(data_list):
        if (query[0] in quote) and (query[1] in quote):
...
elif("and" not in query) and ("or" not in query):     # when the user does not enter any operations, searching for two items will apply AND condition
    print("Performing AND search for: ", query)
    for i,quote in enumerate(data_list):
        if (query[0] in quote) and (query[1] in quote):
            print("Found at", i, quote[0:])
